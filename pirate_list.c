...
void merge(pirate **left, int leftLen, pirate **right, int rightLen) {
...
void merge_sort(pirate **pirates, int len) {
    if (len < 2) {
        return;
    }
...
void list_sort(pirate_list *pirates){
    ...
    merge_sort(array, list_length(pirates));
}
...
void list_destroy(pirate_list *pirates){
    ...
    // free array of pointers to pirates first
    while (list_length(pirates) > 0){
        list_remove(pirates, list_access(pirates, list_length(pirates) -1 ));
    }
